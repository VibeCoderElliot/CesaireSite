...
function offerCard(o) {
 ...
 return '<article class="card"><div class="card-top"><span class="monogram" aria-hidden="true">'+e(initials(org.name))+'</span>'+btn('favorite',o.id,saved(o.id)?'♥':'♡','icon-button','aria-label="Enregistrer ce stage" aria-pressed="'+saved(o.id)+'"')+'</div><div class="tags"><span class="tag">'+e(org.sector)+'</span>'+(org.featured?'<span class="badge">À découvrir</span>':'')+'</div>'+(o.sample?'<span class="demo-label">Offre fictive · démonstration</span>':'')+'<div><h3><a href="#stage/'+e(o.id)+'">'+e(o.title)+'</a></h3><p class="org-name">'+e(org.name)+'</p></div><p class="description">'+e(o.description)+'</p><div class="meta"><span>⌖ '+e(org.metro||'Accès non précisé')+'</span><span>▦ '+e(period(o))+'</span><span>◷ '+e(o.duration||'Durée selon la période')+'</span></div><div class="card-bottom"><span class="tag '+(open?'green':'')+'">'+(open?M.remaining(state,o)+' place(s)':'Indisponible')+'</span><a href="#stage/'+e(o.id)+'">Voir le stage ↗</a></div></article>';
}
...
function detail(id) {
 ...
 return '<a class="back" href="#explorer">← Tous les stages</a>'+(o.sample?'<p class="demo-notice"><strong>Offre fictive.</strong> Elle sert uniquement à montrer le fonctionnement du site, n’émane pas de l’organisation citée et ne constitue pas un partenariat.</p>':'')+title(e(o.title),e(g.name),'DÉCOUVRIR · '+e(g.sector))+
 '<div class="detail-grid"><div><section class="panel"><h2>Une expérience à découvrir</h2><p class="prewrap">'+e(o.description)+'</p><h3>Ce que tu feras</h3>'+list(o.tasks)+'<h3>Ce qu’on attend de toi</h3>'+list(o.requirements)+'</section><section class="panel"><h2>'+e(g.name)+'</h2><p>'+e(g.description)+'</p><p class="muted">'+e(g.address)+'</p><div class="row-actions"><a class="text-link" href="#organisation/'+e(g.id)+'">Voir l’organisation</a>'+(g.latitude?'<a class="text-link" href="#carte">Voir sur la carte</a>':'')+'</div></section></div><aside class="panel detail-sidebar"><h2>En pratique</h2><dl><dt>Période</dt><dd>'+e(period(o))+'</dd><dt>Durée</dt><dd>'+e(o.duration||'Selon la période')+'</dd><dt>Horaires</dt><dd>'+e(o.schedule||'À convenir')+'</dd><dt>Accès</dt><dd>⌖ '+e(g.metro||'À préciser')+'</dd><dt>Places restantes</dt><dd>'+M.remaining(state,o)+'</dd></dl>'+btn('apply',id,already?'Candidature enregistrée':'Candidater localement','button',(!M.available(state,o)||already?'disabled':''))+btn('favorite',id,saved(id)?'♥ Enregistré':'♡ Enregistrer')+'<p class="form-note">Aucune candidature n’est envoyée à une entreprise. Ceci est une simulation.</p></aside></div>';
}
function organizations(id) {
 if(id) {
  ...
  return '<a class="back" href="#organisations">← Les organisations</a>'+title(e(o.name),e(o.description),e(o.sector))+
   (o.sample?'<p class="demo-notice"><strong>Organisation réelle, fiche de démonstration.</strong> L’adresse est publique, mais les offres sont fictives et aucun partenariat n’est revendiqué.</p>':'')+'<section class="panel"><div class="section-head"><div><p>'+e(o.address)+'</p><p>⌖ '+e(o.metro||'Accès à préciser')+'</p></div>'+ (M.manage(state,o)?'<a class="button" href="#espace/'+e(o.id)+'">Gérer cette organisation</a>':!o.owner?btn('claim',id,'Demander à gérer cette fiche'):tag('approved'))+'</div><div class="row-actions">'+(o.latitude?'<a class="text-link" href="#carte">Voir sur la carte</a>':'')+(o.source?'<a class="text-link" href="'+e(o.source)+'" target="_blank" rel="noopener noreferrer">Source de l’adresse ↗</a>':'')+'</div></section><div class="cards">'+(rows.length?rows.map(offerCard).join(''):empty('Pas encore de stage','Cette organisation n’a pas de stage visible pour le moment.'))+'</div>';
 }
 ...
 return title('Des rencontres qui ouvrent des portes.','Explore les organisations du catalogue ou ajoute ta structure.')+
  '<div class="section-head"><p>'+rows.length+' organisations</p><div class="row-actions"><a class="secondary" href="#carte">Voir la carte</a>'+btn('org-new','','Ajouter une organisation','button')+'</div></div><div class="cards">'+rows.map(o=>'<article class="card"><div class="card-top"><span class="monogram">'+e(initials(o.name))+'</span><span class="tag">'+e(o.sector)+'</span></div>'+(o.sample?'<span class="demo-label">Organisation réelle · fiche non revendiquée</span>':'')+'<h3><a href="#organisation/'+e(o.id)+'">'+e(o.name)+'</a></h3><p class="description">'+e(o.description)+'</p><p class="meta">⌖ '+e(o.metro||'À préciser')+'</p><div class="card-bottom"><a href="#organisation/'+e(o.id)+'">Voir la fiche ↗</a>'+(!o.owner?btn('claim',o.id,'Gérer cette fiche','secondary small'):'<span class="tag">Fiche attribuée</span>')+'</div></article>').join('')+'</div>';
}
function mapView() {
 const rows=state.orgs.filter(o=>o.status==='approved'&&Number.isFinite(Number(o.latitude))&&Number.isFinite(Number(o.longitude)));
 if(!rows.length)return title('La carte se construit.','Les organisations apparaîtront lorsqu’une position aura été ajoutée.')+empty('Aucune position disponible','Revenez bientôt.','#organisations','Voir les organisations');
 const lats=rows.map(o=>Number(o.latitude)),lons=rows.map(o=>Number(o.longitude));
 const minLat=Math.min(...lats),maxLat=Math.max(...lats),minLon=Math.min(...lons),maxLon=Math.max(...lons),latSpan=Math.max(maxLat-minLat,.002),lonSpan=Math.max(maxLon-minLon,.002);
 const point=o=>({x:8+84*(Number(o.longitude)-minLon)/lonSpan,y:8+84*(maxLat-Number(o.latitude))/latSpan});
 const markers=rows.map(o=>{const p=point(o);return '<a href="#organisation/'+e(o.id)+'" class="map-marker"><circle cx="'+p.x.toFixed(2)+'" cy="'+p.y.toFixed(2)+'" r="3.4"><title>'+e(o.name)+'</title></circle><text x="'+p.x.toFixed(2)+'" y="'+p.y.toFixed(2)+'" dx="4.6" dy="1.5">'+e(o.name.slice(0,24))+'</text></a>';}).join('');
 const items=rows.map(o=>{const osm='https://www.openstreetmap.org/?mlat='+Number(o.latitude).toFixed(6)+'&mlon='+Number(o.longitude).toFixed(6)+'#map=17/'+Number(o.latitude).toFixed(6)+'/'+Number(o.longitude).toFixed(6),google='https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(o.name+', '+o.address);return '<article class="map-item" id="map-org-'+e(o.id)+'">'+(o.sample?'<span class="demo-label">Fiche de démonstration</span>':'')+'<h2>'+e(o.name)+'</h2><p>'+e(o.address)+'</p><p>'+e(o.metro)+'</p><div class="map-links"><a href="'+google+'" target="_blank" rel="noopener noreferrer">Google Maps ↗</a><a href="'+osm+'" target="_blank" rel="noopener noreferrer">OpenStreetMap ↗</a></div>'+(o.source?'<a href="'+e(o.source)+'" target="_blank" rel="noopener noreferrer">Source de l’adresse ↗</a>':'')+'</article>';}).join('');
 return title('Les organisations sur la carte.','Positions issues d’adresses publiques. Les offres de stage présentées sont fictives et aucun partenariat n’est revendiqué.','AUTOUR DU LYCÉE AIMÉ CÉSAIRE')+'<div class="map-layout"><section class="panel map-panel" aria-label="Carte schématique des organisations"><svg viewBox="0 0 100 100" role="img" aria-label="Carte des organisations autour de Clisson"><path class="map-water" d="M-5 73 C18 54 27 75 46 56 S75 43 105 23"/><path class="map-road" d="M3 20 C27 33 38 32 55 48 S77 79 102 86"/><path class="map-road" d="M16 101 C30 73 48 66 65 37 S82 12 91 -4"/>'+markers+'</svg></section><section class="map-list" aria-label="Adresses">'+items+'</section></div>';
}
...
function render() {
 ...
 else if(route==='carte') content=mapView();
...
function orgForm(id='') {
 ...
 dialog(id?'Modifier l’organisation':'Ajouter une organisation',form('org-save',field('name','Nom de l’organisation',g.name)+field('sector','Secteur',g.sector)+field('metro','Accès / transports',g.metro,'text',false)+field('address','Adresse',g.address)+field('email','Email de contact fictif',g.email,'email',false)+field('phone','Téléphone fictif',g.phone,'tel',false)+area('description','Présentation',g.description,true),id,'',id?'Enregistrer':'Créer la fiche locale'));
}
